...
create_directory() {
    ...
    [[ ! -d "${yzto_dir}/info" ]] && mkdir -p ${yzto_dir}/info
}
...
email_set() {
    if [[ "on" != ${old_config_status} ]]; then
        ...
        [yY][eE][sS] | [yY])
            read -r -p "请输入合法的email (e.g. me@yzto.com):" custom_email
            ...
            ;;
        *)
            custom_email="$(head -n 10 /dev/urandom | md5sum | head -c ${random_num})@yzto.com"
            ;;
        esac
...
modify_nginx_origin_conf() {
    ...
    sed -i '$i include /etc/yzto/conf/nginx/*.conf;' ${nginx_dir}/conf/nginx.conf
...
auto_update() {
    ...
    if [[ ! -f ${auto_update_file} ]] || [[ $(crontab -l | grep -c "auto_update.sh") -lt 1 ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N -P ${yzto_dir} --no-check-certificate https://raw.githubusercontent.com/paniy/Xray_bash_onekey/main/auto_update.sh && chmod +x ${auto_update_file}
...
ssl_install() {
    ...
    ##兼容代码，未来删除
    [[ ${custom_email} == "" ]] && read_optimize "请输入注册域名的邮箱 (e.g. me@yzto.com):" "custom_email" "NULL"
...
domain_check() {
    ...
    read_optimize "请输入你的域名信息 (e.g. www.yzto.com):" "domain" "NULL"
...
old_config_input() {
    ...
    ##兼容代码，未来删除
    [[ ${custom_email} == null ]] && custom_email="$(head -n 10 /dev/urandom | md5sum | head -c ${random_num})@yzto.com"
...
server {
    ...
    ssl_certificate       /etc/yzto/cert/xray.crt;
    ssl_certificate_key   /etc/yzto/cert/xray.key;
    ...
    root /403.html;
    error_page 403 https://www.yzto.com/helloworld;
...
server {
    ...
    listen [::]:80;
    server_name serveraddr.com;
    return 301 https://www.yzto.com\$request_uri;
}
EOF
    if [[ ${bt_nginx} == "Yes" ]]; then
...
acme_cron_update() {
    ...
    if [[ -f ${ssl_update_file} ]] && [[ $(crontab -l | grep -c "ssl_update.sh") == "1" ]]; then
        ...
        *)
            if [[ "${ssl_self}" != "on" ]]; then
                wget -N -P ${yzto_dir} --no-check-certificate https://raw.githubusercontent.com/paniy/Xray_bash_onekey/main/ssl_update.sh && chmod +x ${ssl_update_file}
...
bbr_boost_sh() {
    if [[ -f "${yzto_dir}/tcp.sh" ]]; then
        cd ${yzto_dir} && chmod +x ./tcp.sh && ./tcp.sh
    else
        wget -N --no-check-certificate -P ${yzto_dir} "https://raw.githubusercontent.com/ylx2016/Linux-NetSpeed/master/tcp.sh" && chmod +x ${yzto_dir}/tcp.sh && ${yzto_dir}/tcp.sh
    fi
}
...
uninstall_all() {
    ...
    read -r remove_all_yzto_file_fq
    case $remove_all_yzto_file_fq in
    [yY][eE][sS] | [yY])
        rm -rf ${yzto_command_file}
        rm -rf ${yzto_dir}
        systemctl daemon-reload
...
update_sh() {
    ...
    if [[ ${shell_version} != ${newest_version} ]]; then
        ...
        [yY][eE][sS] | [yY])
            [[ -L ${yzto_command_file} ]] && rm -f ${yzto_command_file}
            wget -N --no-check-certificate -P ${yzto_dir} https://raw.githubusercontent.com/paniy/Xray_bash_onekey/main/install.sh && chmod +x ${yzto_dir}/install.sh
...
menu() {
    ...
    echo -e "--- changed by www.yzto.com ---"
